...
from backend.app.models.approval_models import ApprovalRequest, ApproveResponse, RejectResponse
...
async def list_approvals(status: str = "pending", user_id: str = Depends(get_current_user)):
...
async def reject_action(id: str, reason: dict, user_id: str = Depends(get_current_user)):
    raise NotImplementedError()
