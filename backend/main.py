...
REQUIRED_SECRETS = ["SECRET_KEY", "FIREBASE_PROJECT_ID", "SUPABASE_JWT_SECRET", "GEMINI_API_KEY", "SERPER_API_KEY"]
for secret in REQUIRED_SECRETS:
    val = os.getenv(secret)
    if not val:
        print(f"CRITICAL ERROR: Missing required environment variable: {secret}", file=sys.stderr)
        sys.exit(1)
...
from backend.app.api.routes import health, swarms, swarms_voice, websocket, approvals, memory, crews, audit, schedules, briefings
